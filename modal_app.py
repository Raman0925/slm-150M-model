        ...
        def _flush():
            ...
            while len(buf) >= seq_len:
                ...
                if win_count % config.VAL_EVERY_N_WINDOWS == 0:
                    fva.write(window.tobytes())
                    n_val += 1
                else:
                    ftr.write(window.tobytes())
                    n_train += 1
                win_count += 1
...
# ---- Phase 5: pretrain the 125M model on GPU (not in the Replication Guide) ----
gpu_image = (
    modal.Image.debian_slim(python_version="3.12")
    .pip_install(
        "torch==2.5.1",
...
@app.function(image=gpu_image, volumes=VOLUMES, gpu=PRETRAIN_GPU, timeout=60 * 60 * 24,
              memory=32_768)
def pretrain(max_steps: int, resume: bool = True, calibrate_steps: int = 0,
    ...
    from train import build_schedule, load_windows, lr_at_step
    ...
    print("loading token windows into RAM...")
    t0 = time.time()
    train_windows = load_windows(config.TRAIN_TOKENS_DIR, config.SEQ_LEN)
    print(f"  train: {train_windows.shape[0]:,} windows "
          f"({train_windows.nbytes/1e9:.2f} GB) in {time.time()-t0:.1f}s")

    # micro_batch_size=32 (config.py's TrainConfig default) targets 8xH100 and OOMs a single
    # A10G's 24GB; override here while keeping global_batch_tokens fixed via more grad-accum steps.
    sched = build_schedule(
        micro_batch_size=micro_batch_size,
        global_batch_tokens=config.TRAIN.global_batch_tokens,
        seq_len=config.SEQ_LEN,
        warmup_tokens=config.TRAIN.warmup_tokens,
    )
    print(f"  schedule: micro_bs={sched.micro_batch_size} accum={sched.grad_accum_steps} "
          f"global_bs={sched.global_batch_size} ({sched.global_batch_size*config.SEQ_LEN/1e3:.0f}K tok/step) "
          f"warmup_steps={sched.warmup_steps}")

    llama_cfg = LlamaConfig(**config.MODEL.to_llama_kwargs())
    llama_cfg._attn_implementation = "sdpa"  # memory-efficient attention (no full seq^2 matrix)
    model = LlamaForCausalLM(llama_cfg).to(device)  # fp32 master weights; bf16 via autocast
    ...
    rng = np.random.default_rng(config.TRAIN.seed + start_step)
    metrics_fh = open(config.METRICS_PATH, "a", encoding="utf-8")

    model.train()
    step_times: list[float] = []
    step = start_step
    stop_at = min(max_steps, start_step + calibrate_steps) if calibrate_steps else max_steps
    while step < stop_at:
        lr = lr_at_step(step, warmup_steps=sched.warmup_steps, max_steps=max_steps,
        ...
        total_loss = 0.0
        for _ in range(sched.grad_accum_steps):
            idx = rng.integers(0, n_windows, size=sched.micro_batch_size)
            batch = torch.from_numpy(train_windows[idx].astype(np.int64)).to(device)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                out = model(input_ids=batch, labels=batch)
            loss = out.loss / sched.grad_accum_steps
            loss.backward()
        ...
        if step % config.TRAIN.log_every_steps == 0 or step == stop_at:
            tok_per_s = sched.global_batch_size * config.SEQ_LEN / dt
            print(f"step {step}/{max_steps} loss={total_loss:.4f} lr={lr:.2e} "
                  f"tok/s={tok_per_s:.0f} dt={dt:.2f}s")
            metrics_fh.write(json.dumps({"step": step, "loss": total_loss, "lr": lr,
                                          "tok_per_s": tok_per_s}) + "\n")
            metrics_fh.flush()

        if not calibrate_steps and (step % config.TRAIN.ckpt_every_steps == 0 or step == max_steps):
    ...
    metrics_fh.close()
    # Drop the first few steps (cuda/cudnn warmup) from the throughput average.
    steady = step_times[3:] if len(step_times) > 3 else step_times
    avg_dt = sum(steady) / len(steady) if steady else 0.0
    tok_per_s = sched.global_batch_size * config.SEQ_LEN / avg_dt if avg_dt else 0.0
    print(f"DONE: {step} steps, steady-state {tok_per_s:.0f} tok/s")
    return {"step": step, "avg_tok_per_s": tok_per_s,
            "global_batch_tokens": sched.global_batch_size, "n_params": n_params}


@app.local_entrypoint()
def pretrain_calibrate(steps: int = 30, micro_batch_size: int = 16):
    """Run a few dozen steps to measure real A10G throughput before committing to a full run."""
    result = pretrain.remote(max_steps=10**9, resume=False, calibrate_steps=steps,
                              micro_batch_size=micro_batch_size)
    tok_per_s = result["avg_tok_per_s"]
    print(f"\nCALIBRATION RESULT: {tok_per_s:,.0f} tokens/sec steady-state on {PRETRAIN_GPU}")
    if tok_per_s > 0:
        secs_per_epoch = config.TARGET_TOKENS / tok_per_s
        print(f"  est. time for 1 epoch over ~{config.TARGET_TOKENS/1e9:.2f}B train tokens: "
              f"{secs_per_epoch/3600:.2f} hr")
        # A10G on-demand price is ~$1.10/hr on Modal.
        print(f"  est. cost for 1 epoch @ $1.10/hr: ${secs_per_epoch/3600*1.10:.2f}")


@app.local_entrypoint()
def pretrain_run(max_steps: int = 3890, resume: bool = True, micro_batch_size: int = 16):
    """max_steps default = 1 epoch over the packed 2.04B-token train set at global_batch=524288."""
    result = pretrain.remote(max_steps=max_steps, resume=resume, calibrate_steps=0,
                              micro_batch_size=micro_batch_size)
    print(result)


# ---- Phase 6: deploy tokenizer + trained checkpoint to Hugging Face Hub ----
...
def deploy() -> dict:
    ...
    if not os.path.exists(config.RESUME_CKPT_PATH):
        raise FileNotFoundError(f"no checkpoint at {config.RESUME_CKPT_PATH}; run pretrain first")
    ...
    for fname in os.listdir(config.TOKENIZER_DIR):
        shutil.copy(f"{config.TOKENIZER_DIR}/{fname}", out_dir)

    api = HfApi(token=os.environ["HUGGINGFACE_TOKEN"])
    api.create_repo(config.HF_REPO, exist_ok=True)
    api.upload_folder(folder_path=out_dir, repo_id=config.HF_REPO)
    print(f"pushed step {step} checkpoint to https://huggingface.co/{config.HF_REPO}")
    return {"step": step, "repo": config.HF_REPO}


@app.local_entrypoint()
def deploy_run():
    result = deploy.remote()
    print(result)
